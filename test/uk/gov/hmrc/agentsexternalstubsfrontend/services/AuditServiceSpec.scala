...
class AuditServiceSpec extends UnitSpec with MockitoSugar with Eventually {
  ...
  "auditService" should {
    ...
    "send an AgentsExternalStubsFrontendSomethingHappened event with the correct fields" in {
      ...
      val model = AgentsExternalStubsFrontendModel(
      ...
      await(
        service
          .sendAgentsExternalStubsFrontendSomethingHappened(model, Arn("ARN0001"))(hc, FakeRequest("GET", "/path")))
      ...
      eventually {
        ...
        sentEvent.auditType shouldBe "AgentsExternalStubsFrontendSomethingHappened"
        sentEvent.auditSource shouldBe "agents-external-stubs-frontend"
        ...
        sentEvent.tags("transactionName") shouldBe "agents-external-stubs-frontend-something-happened"
