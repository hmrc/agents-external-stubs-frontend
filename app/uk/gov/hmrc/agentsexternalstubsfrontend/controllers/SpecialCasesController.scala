...
class SpecialCasesController @Inject() (
  ...
  def showAllSpecialCasesPage(caseId: Option[String]): Action[AnyContent] = Action.async { request =>
  ...
  def showEditSpecialCasePage(caseId: Option[String]): Action[AnyContent] = Action.async { request =>
    given Request[AnyContent] = request
    authorised()
      .retrieve(Retrievals.credentialsWithPlanetId) { credentials =>
        caseId match {
  ...
  def upsertSpecialCase(caseId: Option[String]): Action[AnyContent] = Action.async { request =>
    given Request[AnyContent] = request
    authorised()
      .retrieve(Retrievals.credentialsWithPlanetId) { credentials =>
        SpecialCaseForm.form
  ...
  def deleteSpecialCase(caseId: String): Action[AnyContent] = Action.async { request =>
    given Request[AnyContent] = request
    authorised()
